...
export default function AccountDetails() {
  ...
  if (!user) return <div>{t("AccountDetails.Not_Connected_desc")}</div>;

  return (
    <div className="max-w-2xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-4">{t("AccountDetails.LastAccountOperations")} #{id}</h2>
      {loading ? (
            <div>{t("AccountDetails.Loading")}</div>
        ) : operations.length > 0 ? (
            <ul>
                {operations.map(op => (
            ...
            <div className="text-red-500">{t("AccountDetails.NoOperationsFound")}</div>
      )}
    </div>
  );
}