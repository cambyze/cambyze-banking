...
function Carousel() {
  ...
  const items = [
    ...
    { 
      title: t('HomePage.carousel.new_Service_Title'), 
      desc: t('HomePage.carousel.new_Service_Desc'),
...
export default function HomePage() {
  ...
  <h3 className="text-xl font-semibold mb-4 text-center">{t("HomePage.Select_an_address")} </h3>
  <div className="flex flex-col items-center gap-4">
    <button
      onClick={() => setModalOpen(true)}
      className="px-6 py-3 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition-colors"
    >
      {t("HomePage.Choose_an_address")}
    </button>

    {selectedAddress && (
      <div className="w-full mt-6 bg-white rounded-lg shadow p-4 border">
        <h4 className="text-lg font-semibold text-gray-800 mb-2">{t("HomePage.modal.Select_Address")} </h4>
    ...
    />
      ...
      <div className="max-w-lg mx-auto my-12 p-6 bg-white rounded-xl shadow border">
        <h3 className="text-xl font-semibold mb-4 text-center">{t("HomePage.Send_Us_Mail")} </h3>
        <SendMailForm />
      </div>
    </div>
  );
...
function SendMailForm() {
  ...
  const handleSend = async (e) => {
    ...
    setResult(null);
    try {
      const params = new URLSearchParams({ to: to, subject: subject, text: body }).toString();
  ...
  return (
    <form className="space-y-3" onSubmit={handleSend}>
      <input
        ...
        placeholder={t("HomePage.mail.to")}
        value={to}
      ...
      <input
        type="text"
        className="w-full px-3 py-2 rounded border"
        placeholder={t("HomePage.mail.subject")}
        value={subject}
      ...
      <textarea
        className="w-full px-3 py-2 rounded border"
        placeholder={t("HomePage.mail.body")}
        value={body}
      ...
      >
        {loading ? t("HomePage.Sending_Mail") : t("HomePage.Mail_Sent") }
      </button>
      {result && (
