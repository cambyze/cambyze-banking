...
public class BankingServices {
  ...
  // Map to store reset codes for password recovery
  private final java.util.Map<String, ResetCodeData> codes = new java.util.HashMap<>();

  public BankingServices(PersistenceServices persistenceServices, JavaMailSender mailSender) {
  ...
  public String createPerson(String name, String firstName, String email, String psw,
    ...
    String per = persistenceServices.createNewPerson(name, firstName, email, psw, 
        street, city, state, country, secondaryAddress);
  ...
  public boolean login(String mail) {
    ...
    LOGGER.debug("pers :: {} ", pers);
    // if (pers == null || pers.size() < 1 || pers.isEmpty()) {
    if (pers == null || pers.isEmpty() || pers.get(0) == null || pers.get(0).getEmail() == null) {
  ...
  public Boolean bankTransfer(String receiver, String sender, BigDecimal amount) {
    if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
      LOGGER.error("Amount must be positive and non-null");
      return false;
    }
    ...
    try {
      ...
      BigDecimal available =
          senderAccount.getBalanceAmount().add(senderAccount.getOverdraftAmount());
      if (roundedAmount.compareTo(available) > 0) {
        LOGGER.error("Insufficient funds for transfer. Available: {}, Requested: {}", available,
            roundedAmount);
        return false;
      }

      CreateWithdrawResponse withdrawResponse = createWithdraw(sender, roundedAmount);
      if (!Constants.SERVICE_OK.equals(withdrawResponse.getReturnCode())) {
        LOGGER.error("Withdraw failed for sender: {}", sender);
        return false;
      }

      CreateDepositResponse depositResponse = createDeposit(receiver, roundedAmount);
      if (!Constants.SERVICE_OK.equals(depositResponse.getReturnCode())) {
        LOGGER.error("Deposit failed for receiver: {}", receiver);
        return false;
      }

      LOGGER.debug("Transfer of {} from {} to {} succeeded", roundedAmount, sender, receiver);
      return true;
    } catch (Exception e) {
  ...
  public boolean forgottenPsw(String mail) {
      ...
      // if (!pers.get(0).getEmail().equals(mail)) {
      //   LOGGER.error("error mail is invalid 2: {} / {} || per: {} / {}", mail, pers.get(0).getEmail());
      //   return false;
      // }
    
      try {
        String newPsw = generateAndSendCode(mail);
        pers.get(0).setPsw(newPsw);
        persistenceServices.updatePerson(pers.get(0));
        sendEmail(mail, "Password Reset", "Your new password is: " + newPsw + ". Please change it after logging in.");
      } catch (Exception e) {
  ...
  private String generateAndSendCode(String mail) {
    List<Person> pers = persistenceServices.findPersonByMail(mail);
    LOGGER.debug("generateAndSendCode called with mail: {}", mail);
  ...
  public Person seeProfileById(String personId) {
    if (personId == null || personId.isEmpty()) {
      LOGGER.debug("PERSON ID IS EMPTY");
      return null;
    }
    Person per = persistenceServices.findPersonByid(personId);
    if (per == null) {
      LOGGER.debug("PERSON IS EMPTY");
      return null;
    }
    LOGGER.debug("PERSON :: {}", per);
    return per;
  }
  ...
  public Person updateProfile(String personId, String name, String firstName, String email, String psw,
    ...
    if (street != null && !street.isEmpty())
      per.getAdress().setStreet(street);
    if (city != null && !city.isEmpty())
      per.getAdress().setCity(city);
    if (state != null && !state.isEmpty())
      per.getAdress().setState(state);
    if (country != null && !country.isEmpty())
      per.getAdress().setCountry(country);
    if (secondaryAddress != null && !secondaryAddress.isEmpty())
      per.getAdress().setSecondaryAddress(secondaryAddress);

    persistenceServices.updatePerson(per);
    LOGGER.debug("PERSON UPDATED :: {}", per);
    return per;
  }
   ...
   * @param amount
   */
  public boolean updateOverdraft(String ban, int amount) {
    if (ban == null || ban.isEmpty()) {
      LOGGER.debug("BAN IS EMPTY");
      return false;
    }
    if (amount < 0) {
    ...
    boolean res = persistenceServices.updateOverdraftAmount(ban, amount);
    LOGGER.debug("OVERDRAFT UPDATED :: {}", res);
    return res;
  }
